...
router.post('/:id/leave', async (req: Request, res: Response) => {
    ...
    try {
        await client.query('BEGIN');
        const { rows } = await client.query<{ clan_id: number, clan_role_id: string }>(
        ...
        if (user.clan_role_id === 'leader') {
            ...
            if (parseInt(membersResult.rows[0].count, 10) > 1) {
                await client.query('ROLLBACK');
                return res.status(403).json({ error: 'Лидер не может покинуть клан. Сначала передайте лидерство.' });
            }
