...
<nav class="grid grid-cols-5 p-2 bg-black/20 backdrop-blur-md border-t border-white/10 flex-shrink-0">
	...
	<button on:click={() => setView('account')} class="nav-button" class:active={activeView === 'account'}>
		...
		<span class="text-xs font-medium">Кабинет</span>
	</button>
</nav>

<style>
	.nav-button {
		@apply flex flex-col items-center justify-center gap-1 p-2 rounded-lg text-text-secondary transition-all duration-200;
	}
	.nav-text {
		@apply text-xs font-medium tracking-wide transition-all duration-200;
	}
	.nav-button.active {
