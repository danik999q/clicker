                ...
                {#each $gameStore.memes as meme, index (meme.id)}
                    ...
                    {#if meme.isUnlocked}
                        <div class="upgrade-item" class:active={index === $gameStore.activeMemeIndex} on:click={() => gameStore.setActiveMeme(index)} role="button" tabindex="0">
                            <div class="upgrade-item-info">
                                ...
                                <p class="item-stats">Ур: {meme.level}</p>
                            </div>
                            <button class="upgrade-button" disabled={$gameStore.totalViews < totalCost || levelsToBuy === 0} on:click|stopPropagation={() => GameService.upgradeMeme(meme.id)}>
                    ...
                    {:else}
                        <div class="upgrade-item locked">
                            ...
                            <button class="unlock-button" disabled={$gameStore.totalViews < meme.unlockCost} on:click={() => GameService.unlockMeme(meme.id)}>
